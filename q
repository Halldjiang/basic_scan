...
# Self-contained q command. It does not need /root/script/basic_scan/check_hostname.py.
# Usage: q R017 | q 10.163.130.31 | q R017 --json
# Optional: PYTHON_BIN=/path/to/python3.12 q R017
# Dependency options:
#   q --install-deps                         install mysql connector into user venv, no sudo
#   Q_AUTO_INSTALL_DEPS=0 q R017             do not auto-install; print instructions instead
#   Q_VENV_DIR=/path/to/venv q --install-deps
...
find_base_python() {
  if [[ -n "${PYTHON_BIN:-}" ]]; then
    printf '%s\n' "$PYTHON_BIN"
  elif [[ -x "$SHARED_VENV/bin/python3" ]]; then
    printf '%s\n' "$SHARED_VENV/bin/python3"
  elif [[ -n "$USER_VENV_DEFAULT" && -x "$USER_VENV_DEFAULT/bin/python3" ]]; then
    printf '%s\n' "$USER_VENV_DEFAULT/bin/python3"
...
print_python_missing() {
  cat >&2 <<'EOF'
Cannot find python3.12 or python3.
Ask an admin to install Python 3.12, or set PYTHON_BIN=/path/to/python3.
EOF
}
...
install_user_deps() {
  local base_py="$1"
  local venv_dir="${2:-$Q_VENV_DIR}"

  if [[ -z "$venv_dir" ]]; then
    echo "Cannot determine user venv directory. Set Q_VENV_DIR=/path/to/venv." >&2
    return 1
  fi
  ...
  if [[ ! -x "$venv_dir/bin/python3" ]]; then
    echo "Creating q Python venv: $venv_dir" >&2
    if ! "$base_py" -m venv "$venv_dir" >/dev/null 2>&1; then
      cat >&2 <<EOF
Failed to create Python virtualenv at:
  $venv_dir

On Ubuntu/Debian this usually means python3.12-venv is missing.
Ask an admin to run one of these once:
  sudo apt-get install python3.12-venv
  sudo ./install_q.sh --global --with-venv

Then normal users can run q without sudo.
EOF
      return 1
    fi
  fi

  echo "Installing mysql-connector-python into: $venv_dir" >&2
...
if [[ "${1:-}" == "--install-deps" ]]; then
  shift
  PYTHON_BIN="$(find_base_python)" || { print_python_missing; exit 2; }
  install_user_deps "$PYTHON_BIN" "$Q_VENV_DIR"
  echo "Dependency install completed. Test with: q R017" >&2
  exit 0
fi

PYTHON_BIN="$(find_base_python)" || { print_python_missing; exit 2; }

# For normal queries, make the MySQL driver available without requiring sudo.
# --help intentionally skips this so help is always fast and offline-safe.
if ! is_help_request "$@"; then
  if ! has_mysql_connector "$PYTHON_BIN"; then
    if [[ "$Q_AUTO_INSTALL_DEPS" == "1" || "$Q_AUTO_INSTALL_DEPS" == "true" || "$Q_AUTO_INSTALL_DEPS" == "yes" ]]; then
      if install_user_deps "$PYTHON_BIN" "$Q_VENV_DIR"; then
        PYTHON_BIN="$Q_VENV_DIR/bin/python3"
      else
        cat >&2 <<'EOF'

q could not install mysql-connector-python automatically.
Manual no-sudo option:
  q --install-deps

Admin shared option for all users:
  sudo ./install_q.sh --global --with-venv
...
Or ask an admin to install a shared venv:
  sudo ./install_q.sh --global --with-venv
EOF
      exit 2
    fi
...
def _is_readable_file(path: Path) -> bool:
    """Return True only when path is a readable regular file.

    Path.is_file() can raise PermissionError when a normal user probes
    legacy root-only locations such as /root/script/basic_scan/Settings.json.
    Those locations must be skipped, not treated as fatal.
    """
    try:
...
def load_settings(path: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not os.access(settings_path, os.R_OK):
            print(
                f"Settings file is not readable by current user, skipping: {settings_path}",
                file=sys.stderr,
            )
            return {}
        ...
        print(f"Failed to check settings file {settings_path}: {exc}", file=sys.stderr)
        return {}
    except Exception as exc:
        print(f"Failed to read settings file {settings_path}: {exc}", file=sys.stderr)
        raise SystemExit(2)
...
def connect_mysql(config: DBConfig):
    if mysql is None:  # type: ignore[name-defined]
        print(
            "Missing dependency: mysql-connector-python\n"
            "For a normal user, run: q --install-deps\n"
            "Or ask an admin to run: sudo ./install_q.sh --global --with-venv\n"
            "No sudo is required for q --install-deps.",
            file=sys.stderr,
        )
        raise SystemExit(2)
